...
from torch_geometric.utils import negative_sampling,add_self_loops, degree, to_undirected
...
import datasets
from impl import SubGDataset, metrics, utils, config
from impl.train import myTrain, myTest
from myModels import SubIGNN_new, VanillaGCN, SoftEIGNN, SoftIGNN, baseline
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='hpo_neuro', help='Path to the dataset')
    ...
    parser.add_argument('--swtich_epoch', type=int, default=200, help='Time to switch to new subgraph')
...
def get_knn_adj(embeddings, k, distance_metric='cosine', normalize_rows=True, binary=True):
    ...
    # distances = np.sort(dist_matrix, axis=1)[:, -k:]
...
def load_and_process_adjs(data_path, k, channel='position'):
    ...
    if channel == 'position':
        adj = np.load(f"/scratch/yongjian/2024/data/{data_path}/subgraph_position_adj.npy")
    elif channel == 'neighbor':
        adj = np.load(f"/scratch/yongjian/2024/data/{data_path}/subgraph_neighbor_adj.npy")
    elif channel == 'structure':
        adj = np.load(f"/scratch/yongjian/2024/data/{data_path}/subgraph_structure_adj.npy")
    else:
...
def get_hybrid_edge_index(
    ...
    # if subgraph_edge_index is None:
    #     adj_hybrid = torch.load(f"adj_hybrid_2_{args.dataset}.pt", weights_only=True)
...
def train(args, 
    ...
    if args.pretrain:
        ...
        for i in range(200):
            trn_score, loss = myTrain(optimizer, pre_train_model, baseG, features, adj_hybrid, score_fn)
        tmp = myTest(pre_train_model, baseG, features, adj_hybrid, score_fn, testing=True)
    ...
    if args.model in ['isnn', 'baseline']:
        if args.pretrain:
            ...
            # subgraph_edge_index = torch.load(f"subgraph_edge_index_{args.dataset}_{args.model}_{args.num_edges}_{args.channel}_{args.gamma}.pt")
            # subgraph_edge_weight = torch.load(f"subgraph_edge_weight_{args.dataset}_{args.model}_{args.num_edges}_{args.channel}_{args.gamma}.pt")
        else:
    ...
    for repeat in range(args.repeat):
        ...
        for i in range(1, args.num_epochs+1):
            ...
            if i == args.swtich_epoch  and args.model in ['isnn'] and args.channel == 'None' and not args.pretrain:
                with torch.no_grad():
                    embeddings = gnn.get_subgraph_embeddings()
                    # embeddings = projection(features[node_mask])
                
                    # subgraph_edge_index, subgraph_edge_weight = get_top_k_adj(embeddings, args.knn, distance_metric='cosine')
                    ...
                    # subgraph_edge_index, subgraph_edge_weight = get_knn_adj(embeddings, args.knn, binary=False, distance_metric='cosine')
                    # import pdb; pdb.set_trace()
                    # torch.save(subgraph_edge_index, f"subgraph_edge_index_{args.dataset}_{args.model}_{args.num_edges}_{args.channel}_{args.gamma}.pt")
                    # torch.save(subgraph_edge_weight, f"subgraph_edge_weight_{args.dataset}_{args.model}_{args.num_edges}_{args.channel}_{args.gamma}.pt")
            ...
            if args.model in ['isnn', 'soft_ignn', 'soft_eignn']:
                ...
                residual_error = 0
            trn_score, trn_auc, loss = myTrain(optimizer, gnn, baseG, features, adj_hybrid, score_fn)
            all_train_scores[repeat].append(trn_score)
            tmp, tmp1 = myTest( gnn, baseG, features, adj_hybrid, score_fn, testing=True)
            all_test_scores[repeat].append(tmp)
            tmp, tmp1 = myTest( gnn, baseG, features, adj_hybrid, score_fn, testing=False)
            all_val_scores[repeat].append(tmp)
            trn_time.append(time.time() - t1)
            # if i > args.swtich_epoch:
            #     scd.step(loss)


            if i >= 200:
                score, auc = myTest( gnn,
                                baseG,
                                features,
                ...
                if score > val_score:
                    ...
                    tst_auc = auc
                    # embeddings = gnn.get_subgraph_embeddings()
                    # torch.save(embeddings, f"embeddings_{args.dataset}_{args.model}_{args.num_edges}_{args.channel}_{args.gamma}.pt")
                    # prediction = gnn.predict(features, adj_hybrid, train_mask=None)
                    # torch.save(prediction, f"prediction_{args.dataset}_{args.model}_{args.num_edges}_{args.channel}_{args.gamma}.pt")

                    print(
                        f"iter {i} loss {loss:.4f} residual {residual_error:.4f} train {trn_score:.4f} val {val_score:.4f} tst {tst_score:.4f} lr {optimizer.param_groups[0]['lr']}",
                        flush=True)
                    ...
                    print(
                        f"iter {i} loss {loss:.4f} residual {residual_error:.4f} train {trn_score:.4f} val {val_score:.4f} tst {score:.4f} lr {optimizer.param_groups[0]['lr']}",
                        flush=True)
                    print(f"Best picked so far- val: {val_score:.4f} tst: {tst_score:.4f}, early stop: {early_stop} \n")
                    ...
                    if i % 10 == 0:
                        inf_start = time.time()
                        tst_1, auc = myTest(gnn, baseG, features, adj_hybrid, score_fn, testing=True)
                        inf_end = time.time()
                        inference_time.append(inf_end - inf_start)
                        print(
                            f"iter {i} loss {loss:.4f} residual {residual_error:.4f} train {trn_score:.4f} val {score:.4f} tst {tst_1:.4f} lr {optimizer.param_groups[0]['lr']}",
                            flush=True)
                        print(
        ...
        # print(
        #     f"end: epoch {i}, train time {sum(trn_time):.2f} s, train {trn_score:.4f} val {val_score:.3f}, tst {tst_score:.3f} lr {optimizer.param_groups[0]['lr']}",
        #     flush=True)
...
if __name__ == "__main__":
    args = parse_args()
    # channels = ['position', 'neighbor', 'structure']
    channels = ['None']
    num_edges = [1]
    ...
    num_clusters = [9]
    swith_epochs = [80]
    args.swtich_epoch = swith_epochs[0]
    kappas = [0.8]
    ...
    for num_cluster in num_clusters:
        ...
        for num_edge in num_edges:
            ...
            args.num_edges = num_edge
            for gamma in gammas:
                args.gamma = gamma
                for knn in knns:
                    args.knn = knn
                    print(f"KNN: {args.knn}, swith_epoch: {args.swtich_epoch}, gamma: {args.gamma}, kappa: {args.kappa}, num_clusters: {args.num_cluster}")
                    run_helper(args)
                    print('\n')
